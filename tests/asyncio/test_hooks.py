...
class TestAsyncHook:
    ...
    def test_is_abstract(cls) -> None:
        """AsyncHook should not be instantiable directly."""
    ...
    def test_must_implement_on_limit(cls) -> None:
        ...
        class IncompleteHook(AsyncHook):
            pass
    ...
    async def test_on_limit__observe_result(cls) -> None:
        """AsyncHook should be able to observe and act on rate limit result."""
        alert_calls: list[str] = []

        class AlertHook(AsyncHook):
...
@pytest.mark.asyncio
class TestBuildAsyncHookChain:
    @classmethod
    async def test_build_async_hook_chain__empty_hooks(
        ...
        chain: Callable[[], Awaitable[RateLimitResult]] = build_async_hook_chain(
    ...
    async def test_on_limit__exception_skips_hook(
        ...
        class FailingHook(AsyncHook):
            async def on_limit(self, *args, **kwargs) -> RateLimitResult:  # noqa: PLR6301
                call_order.append("failing_before")
                raise RuntimeError("Hook failed!")

        class WorkingHook(AsyncHook):
            async def on_limit(self, *args, **kwargs) -> RateLimitResult:  # noqa: PLR6301
                call_next: Callable[[], Awaitable[RateLimitResult]] = args[0]
    ...
    async def test_on_limit__next_fn_raises_before_except_fallback(
        ...
        class PassthroughHook(AsyncHook):
            async def on_limit(self, *args, **kwargs) -> RateLimitResult:  # noqa: PLR6301
                call_next: Callable[[], Awaitable[RateLimitResult]] = args[0]
    ...
    async def test_on_limit__next_fn_raises_after_except_fallback(
        ...
        class FailBeforeCallNextHook(AsyncHook):
            async def on_limit(self, *args, **kwargs) -> RateLimitResult:  # noqa: PLR6301
